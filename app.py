...
# ── Nav helper (แก้ใหม่ ไม่ใช้ HTML ซ้อนกัน) ──
def nav_item(icon, label, page_key, indent=False):
    prefix = "      " if indent else ""
    active_mark = "🔹" if st.session_state.current_page == page_key else "  "
    
    if st.sidebar.button(f"{active_mark} {icon} {prefix}{label}", key=f"nav_{page_key}", use_container_width=True):
...
def nav_group(icon, label, open_key):
    ...
    if st.sidebar.button(f" {icon}  {label}  {arrow}", key=f"grp_{open_key}", use_container_width=True):
        st.session_state[open_key] = not st.session_state[open_key]
        st.rerun()

# ── Section: หน้าหลัก ──
st.sidebar.markdown("""<div style="padding:4px 16px; font-size:0.62rem; color:#475569; letter-spacing:1px; text-transform:uppercase; margin-top:8px;">เมนูหลัก</div>""", unsafe_allow_html=True)
nav_item("🏠", "Dashboard", "dashboard")

# ── Section: ออกแบบส่วนผสม (expandable) ──
st.sidebar.markdown("""<div style="padding:4px 16px; font-size:0.62rem; color:#475569; letter-spacing:1px; text-transform:uppercase; margin-top:12px;">การออกแบบ</div>""", unsafe_allow_html=True)
...
# ── Section: ผลลัพธ์ ──
st.sidebar.markdown("""<div style="padding:4px 16px; font-size:0.62rem; color:#475569; letter-spacing:1px; text-transform:uppercase; margin-top:12px;">ผลการคำนวณ</div>""", unsafe_allow_html=True)
...
# ── Section: อื่นๆ ──
st.sidebar.markdown("""<div style="padding:4px 16px; font-size:0.62rem; color:#475569; letter-spacing:1px; text-transform:uppercase; margin-top:12px;">อื่นๆ</div>""", unsafe_allow_html=True)
...
if st.button("⚙️  ประมวลผลส่วนผสมคอนกรีต (CALCULATE)", type="primary", use_container_width=True):
    ...
    if fm_cube > 100.0 and "Hemp" not in agg_type:
        ...
        # ── SUCCESS BANNER (Fix Markdown indent issue) ──
        st.markdown(
            "<div style='background:linear-gradient(135deg,rgba(16,185,129,0.12),rgba(5,150,105,0.06)); border:1px solid rgba(16,185,129,0.3); border-radius:12px; padding:16px 24px; margin:20px 0; display:flex; align-items:center; gap:12px;'>"
        ...
        # ── MIX RATIO BOX (Fix Markdown indent issue) ──
        st.markdown(
            "<div class='mix-ratio-box'>"
            "<div class='result-title'>อัตราส่วนผสม (Mix Ratio by Weight)</div>"
            f"<div class='mix-ratio-val'>1 : {(fac/cm_total):.2f} : {(cac/cm_total):.2f}</div>"
            f"<div class='mix-ratio-label'>วัสดุประสานรวม : ทราย : หิน &nbsp;|&nbsp; W/C = {wc:.3f} &nbsp;|&nbsp; Wdcc = {wdcc:.0f} kg/m³</div>"
            "</div>",
            unsafe_allow_html=True
        )
        ...
        with out_col1:
            ...
            if scm_pct > 0:
                bat_rows["วัสดุ"].append(scm_type[:15]+".."); bat_rows["Batched (kg/m³)"].append(f"{scm_weight:.1f}"); bat_rows["หมายเหตุ"].append("—")
            ...
            if scm_pct > 0:
                tm_rows["วัสดุ"].append(scm_type[:15]+".."); tm_rows["ปริมาณ (kg)"].append(f"{scm_weight*trial_mix_vol:.3f}")
            ...
            # ── QC TARGET BOX (Fix Markdown indent issue) ──
            scale_html = ""
            if "รวม" in scale_mode:
                scale_html = f"<div style='font-size:0.88rem; color:#94a3b8; margin-top:4px;'>เป้าหมายชั่งรวมโมลด์: <span style='color:#f1f5f9; font-weight:600; font-family:JetBrains Mono,monospace;'>{target_scale_weight_per_mold:.3f} kg</span></div>"
            ...
            st.markdown(
                ...
                f"<div style='font-size:0.88rem; color:#94a3b8;'>น้ำหนักปูนสด/ก้อน (ทฤษฎี): <span style='color:#f1f5f9; font-weight:600; font-family:JetBrains Mono,monospace;'>{fresh_conc_weight_per_mold:.3f} kg</span></div>"
                f"{scale_html}"
                f"<div style='font-size:0.78rem; color:#475569; margin-top:6px;'>อ้างอิง Wdcc = {wdcc:.0f} kg/m³</div>"
                "</div>",
                unsafe_allow_html=True
            )
            ...
            if scm_pct > 0:
                boq_rows["รายการ"].insert(1, scm_type[:15]+".."); boq_rows["ปริมาณรวม"].insert(1, f"{scm_weight*project_volume/1000:,.2f} ตัน")
            st.dataframe(pd.DataFrame(boq_rows), use_container_width=True, hide_index=True)
            
            # ── TOTAL BUDGET BOX (Fix Markdown indent issue) ──
            st.markdown(
                ...
                f"<div style='font-size:1.8rem; font-weight:700; color:#f1f5f9; font-family:JetBrains Mono,monospace; margin-top:4px;'>฿{total_project_cost:,.2f}</div>"
                f"<div style='font-size:0.8rem; color:#64748b; margin-top:2px;'>= ฿{cost_m3:,.2f}/m³ × {project_volume:,.0f} m³</div>"
                "</div>",
                unsafe_allow_html=True
            )
