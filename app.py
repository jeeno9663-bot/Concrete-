...
def nav_item(icon, label, page_key, indent=False):
    ...
    """, unsafe_allow_html=True)
    if st.sidebar.button(f"{label}", key=f"nav_{page_key}", use_container_width=True, label_visibility="collapsed"):
...
def nav_group(icon, label, open_key):
    ...
    """, unsafe_allow_html=True)
    if st.sidebar.button(f"toggle_{open_key}", key=f"grp_{open_key}", use_container_width=True, label_visibility="collapsed"):
